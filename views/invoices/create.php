<?php
$pageTitle   = 'Nouvelle facture — ' . APP_NAME;
$breadcrumbs = ['Factures' => 'invoices', 'Nouvelle facture' => ''];
require __DIR__ . '/../layout/header.php';

// Pre-encode products list for JS
...
// Restore submitted items (on validation error)
$submittedItems = [];
...
<form method="POST" action="<?= APP_URL ?>/invoices/store" id="invoiceForm" novalidate>
  <div class="row g-3">
    ...
    <div class="col-12">
      <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
          <div class="row g-3">
            <div class="col-md-3">
              <label class="form-label fw-semibold">N° Facture <span class="text-danger">*</span></label>
              <input type="text" name="invoice_number" class="form-control" required
                     value="<?= e($old['invoice_number'] ?? $nextNum) ?>">
            </div>
            <div class="col-md-3">
      ...
      <div class="card border-0 shadow-sm h-100">
        ...
        <div class="card-body">
          <textarea name="notes" class="form-control" rows="5"
                    placeholder="Conditions de paiement, remarques…"><?= e($old['notes'] ?? '') ?></textarea>
        </div>
      </div>
    </div>
    ...
    <div class="col-12 d-flex justify-content-end gap-2">
      <a href="<?= APP_URL ?>/invoices" class="btn btn-outline-secondary">Annuler</a>
...
const TAX_RATE    = parseFloat(document.getElementById('taxRate').value) || 20;
const SUBMITTED   = {$submittedItemsJson};

document.addEventListener('DOMContentLoaded', () => {
  // Restore submitted items (validation error) or add 1 empty row
...
function addRow(item = {}) {
  ...
  const productOptions = PRODUCTS.map(p =>
    `<option value="\${p.id}" data-price="\${p.unit_price}" data-qty="\${p.quantity}">\${p.name} (\${p.sku})</option>`
  ).join('');
  ...
  tr.innerHTML = `
    ...
    <td>
      <span class="item-total fw-semibold">0,00 MAD</span>
    </td>
    <td>
  ...
  // Pre-select product if restoring
  if (item.product_id) {
...
function recalc() {
  ...
  const ttc = totalHT + tax;

  document.getElementById('displayHT').textContent  = formatMAD(totalHT);
  document.getElementById('displayTax').textContent = formatMAD(tax);
  document.getElementById('displayTTC').textContent = formatMAD(ttc);
}
