<?php
$pageTitle   = 'Modifier facture — ' . APP_NAME;
$breadcrumbs = ['Factures' => 'invoices', 'Modifier' => ''];
require __DIR__ . '/../layout/header.php';

$invoiceId = (int) ($old['id'] ?? $_GET['id'] ?? 0);

// Items to render: on error use POST, otherwise use DB items
...
<div class="d-flex align-items-center gap-2 mb-4">
  <a href="<?= APP_URL ?>/invoices/show?id=<?= $invoiceId ?>" class="btn btn-sm btn-outline-secondary">
    <i class="bi bi-arrow-left"></i>
  </a>
  <h4 class="fw-bold mb-0">Modifier la facture</h4>
</div>
...
<form method="POST" action="<?= APP_URL ?>/invoices/update" id="invoiceForm" novalidate>
  ...
  <div class="row g-3">
    ...
    <div class="col-md-7">
      <div class="card border-0 shadow-sm h-100">
        ...
        <div class="card-body">
          <textarea name="notes" class="form-control" rows="5"><?= e($old['notes'] ?? '') ?></textarea>
        </div>
      </div>
    </div>
      ...
      <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
          <table class="table table-sm mb-0">
            ...
            <tr>
              <td class="text-muted">TVA (<span id="displayRate"><?= e($old['tax_rate'] ?? TAX_RATE) ?></span>%)</td>
    ...
    <div class="col-12 d-flex justify-content-end gap-2">
      <a href="<?= APP_URL ?>/invoices/show?id=<?= $invoiceId ?>" class="btn btn-outline-secondary">Annuler</a>
...
document.addEventListener('DOMContentLoaded', () => {
  INIT_ITEMS.forEach(item => addRow(item));
  recalc();
  ...
  tr.innerHTML = `
    <td>
      <input type="text" name="items[\${idx}][label]" class="form-control form-control-sm item-label"
             placeholder="Description" required value="\${escHtml(item.label||'')}">
    </td>
    <td>
    ...
    <td>
      <input type="number" name="items[\${idx}][unit_price]" class="form-control form-control-sm item-price"
             min="0" step="0.01" value="\${parseFloat(item.unit_price||0).toFixed(2)}" required>
    </td>
