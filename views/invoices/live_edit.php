...
<head>
  ...
  <title>Édition — Facture <?= e($invoice['invoice_number']) ?></title>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .doc-date { font-size: 10px; color: #555; margin-top: 3px; }
    ...
    .info-row { display: flex; gap: 32px; flex-wrap: wrap; border-top: 1px solid #ddd; padding-top: 12px; margin-top: 12px; }
    .info-item .ii-label { font-size: 9px; text-transform: uppercase; letter-spacing: .5px; color: #888; }
...
<div class="le-toolbar no-print">
  <div class="le-toolbar-left">
    <a href="<?= APP_URL ?>/invoices/show?id=<?= $invoice['id'] ?>" class="le-btn">← Retour</a>
    <span class="le-title">Édition — <strong>Facture <?= e($invoice['invoice_number']) ?></strong></span>
  </div>
  <div class="le-toolbar-right">
    ...
    <label style="font-size:11px;color:#94a3b8;">Règlement</label>
    <select class="le-toolbar-select" data-field="payment_method">
      <option value="">—</option>
      <option value="cheque"   <?= ($invoice['payment_method']??'')==='cheque'   ?'selected':'' ?>>Chèque</option>
      <option value="espece"   <?= ($invoice['payment_method']??'')==='espece'   ?'selected':'' ?>>Espèce</option>
      <option value="virement" <?= ($invoice['payment_method']??'')==='virement' ?'selected':'' ?>>Virement</option>
    </select>
    ...
    <a class="le-btn" href="<?= APP_URL ?>/invoices/pdf?id=<?= $invoice['id'] ?>">PDF</a>
  </div>
</div>
...
<div class="page">
  ...
  <div class="doc-header">
    ...
    <div class="doc-title-block">
      ...
      <div class="doc-ref">
        <span class="le-editable" contenteditable="true" data-field="invoice_number"
              style="font-size:13px;font-weight:600;"><?= e($invoice['invoice_number']) ?></span>
      </div>
      <div class="doc-date">
        <input type="date" class="le-input le-input-date" data-field="date"
               value="<?= e($invoice['date']) ?>">
      </div>
    </div>
  </div>
  ...
  <div class="client-block">
    ...
    <div class="client-name"><?= e($invoice['client_name']) ?></div>
    <div class="client-detail">
      <?php if ($invoice['client_address']): ?>
        <?= nl2br(e($invoice['client_address'])) ?><br>
      <?php endif; ?>
      <?php if ($invoice['client_ice']): ?>ICE&nbsp;: <?= e($invoice['client_ice']) ?><?php endif; ?>
    </div>
  </div>
  <div class="cf"></div>
        ...
        <th class="r" style="width:18%">
          ...
          <span class="no-print" style="font-weight:400;font-size:9px;margin-left:6px;">
            TVA&nbsp;<input type="number" id="edTaxRate" class="le-input le-recalc le-input-sm"
                            data-field="tax_rate" value="<?= (float)$invoice['tax_rate'] ?>">%
          </span>
        </th>
      ...
      <tr class="le-item-row" data-product-id="<?= (int)($item['product_id'] ?? 0) ?>">
        <td>
          <span class="le-label le-editable" contenteditable="true"
                style="display:block;min-width:80px;"><?= e($item['label']) ?></span>
        </td>
        <td class="no-print" style="text-align:center;">
          <button type="button" class="le-row-del" onclick="removeItemRow(this)" title="Supprimer">✕</button>
        </td>
        <td class="r">
          <input type="number" class="le-input le-qty le-recalc"
                 value="<?= (float)$item['quantity'] ?>" min="0.01" step="0.01" style="width:56px">
        </td>
        <td class="r">
          <input type="number" class="le-input le-price le-recalc"
  ...
  <button type="button" class="le-add-row-btn no-print"
          onclick="addItemRow('itemsTbody', true)">+ Ajouter une ligne</button>
  ...
  <div class="totals-wrap">
    <table class="totals-table">
      <tr>
        <td>Total HT</td>
        <td id="sumHT"><?= formatMoney((float)$invoice['total_ht']) ?></td>
      </tr>
      <tr>
        <td>TVA (<span id="displayTaxRate"><?= $invoice['tax_rate'] ?></span>%)</td>
        <td id="sumTax"><?= formatMoney((float)$invoice['tax_amount']) ?></td>
      </tr>
      <tr class="total-net">
        <td>Total Net à payer</td>
        <td id="sumTTC"><?= formatMoney((float)$invoice['total_ttc']) ?></td>
      </tr>
    </table>
  </div>
  ...
  <div class="amount-words">
    ...
    <span id="sumWords"><?= e(amountInWords((float)$invoice['total_ttc'])) ?></span>
  </div>

  <!-- Notes -->
  <div class="notes-block" style="min-height:28px;">
    <div class="le-editable-block" contenteditable="true" data-field="notes"
         placeholder="Notes…"><?= e($invoice['notes']) ?></div>
  </div>

  <!-- Signature -->
  <?php
    $dp = $invoice['date'] ? explode('-', $invoice['date']) : null;
    $df = $dp ? $dp[2].' / '.$dp[1].' / '.$dp[0] : '___ / ___ / ______';
  ?>
  <div class="sig-row">
    <div class="sig-col">
      <div class="sig-lbl">Fait à _____________________, le <?= $df ?></div>
      <div class="sig-place">
        <?php if (!empty($invoice['use_watermark']) && !empty($company['watermark_data_uri'])): ?>
...
<script>
var LE_CONFIG = { docId: <?= $invoice['id'] ?>, saveUrl: '<?= APP_URL ?>/invoices/live-update' };
</script>
</body>
</html>
