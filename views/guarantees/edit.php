<?php
$pageTitle   = 'Modifier certificat — ' . APP_NAME;
$breadcrumbs = ['Garanties' => 'guarantees', 'Modifier' => ''];
...
// Restore items: prefer POST (on validation error), otherwise DB rows
$submittedItemsJson = json_encode(
    !empty($_POST['items'])
        ? array_values($_POST['items'])
        : array_map(fn($i) => [
            'label'      => $i['label'],
            'quantity'   => $i['quantity'],
...
<div class="d-flex align-items-center gap-2 mb-4">
  <a href="<?= APP_URL ?>/guarantees/show?id=<?= $cert['id'] ?>" class="btn btn-sm btn-outline-secondary">
    <i class="bi bi-arrow-left"></i>
  </a>
  <h4 class="fw-bold mb-0"><i class="bi bi-shield-check me-2 text-primary"></i>Modifier le certificat</h4>
</div>
...
<form method="POST" action="<?= APP_URL ?>/guarantees/update" novalidate>
  ...
  <div class="row g-3">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-3">
              ...
              <input type="text" name="certificate_number" class="form-control" required
                     value="<?= e($old['certificate_number'] ?? '') ?>">
            </div>
    ...
    <div class="col-12">
      <div class="card border-0 shadow-sm">
        <div class="card-header bg-white fw-semibold">Description générale</div>
        <div class="card-body">
    ...
    <div class="col-md-7">
      <div class="card border-0 shadow-sm h-100">
        ...
        <div class="card-body">
          <textarea name="terms" class="form-control" rows="8"><?= e($old['terms'] ?? '') ?></textarea>
        </div>
      </div>
    </div>
    ...
    <div class="col-12 d-flex justify-content-end gap-2">
      <a href="<?= APP_URL ?>/guarantees/show?id=<?= $cert['id'] ?>" class="btn btn-outline-secondary">Annuler</a>
...
function addRow(item) {
  ...
  tr.innerHTML =
    ...
    '<input type="hidden" name="items[' + idx + '][product_id]" class="item-product-id" value="' + (item.product_id || '') + '"></td>' +
...
function recalc() {
  ...
  document.querySelectorAll('#itemsBody tr').forEach(function (tr) {
    ...
    const pEl  = tr.querySelector('.item-price');
    const span = tr.querySelector('.item-total');
    if (pEl && pEl.value !== '') {
      const line = qty * parseFloat(pEl.value);
      total += line; hasPrice = true;
  ...
  block.style.display = hasPrice ? '' : 'none';
  if (hasPrice) document.getElementById('displayTotal').textContent = formatMAD(total);
}
...
function updateDuration() {
  ...
  if (!document.getElementById('startDate').value || !document.getElementById('endDate').value || e <= s) { badge.textContent = ''; return; }
  const days = Math.round((e - s) / 86400000);
  const months = Math.round(days / 30.44);
  badge.textContent = 'Durée : ' + (months >= 12 ? Math.floor(months / 12) + ' an(s)' + (months % 12 ? ' ' + (months % 12) + ' mois' : '') : months + ' mois') + ' (' + days + ' jours)';
}
